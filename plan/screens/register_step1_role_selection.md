# Registration Flow - Step 1: Role Selection

## 1. Screen Context & Flow Position
- **Screen Name & ID**: `RegisterRoleSelectionScreen`
- **Flow Position**: Step 1 of Registration.
- **Route & Arguments**: `Route.RegisterRoleSelection`
...
```kotlin
data class RegisterRoleSelectionScreenUiState(
    val isLoading: Boolean = false,
    val error: String? = null
)

sealed interface RegisterRoleSelectionScreenUiEvent {
    object LoadData : RegisterRoleSelectionScreenUiEvent
}

@HiltViewModel
class RegisterRoleSelectionScreenViewModel @Inject constructor(
    private val repository: DefaultRepository
) : ViewModel() {
    private val _state = MutableStateFlow(RegisterRoleSelectionScreenUiState())
...
fun RegisterRoleSelectionScreen(
    viewModel: RegisterRoleSelectionScreenViewModel = hiltViewModel(),
...
> UI Design of a Role Selection screen for a pet rescue app. Dark mode. 'Choose your role' header. A vertical list of large cards for Adopter, Foster, Transporter, Veterinarian. Each card has an icon and is slate grey, but the selected one has a cyan border.
