...
- **Screen Name & ID**: `AdoptionApplicationScreen`
- **Flow Position**: Applying for a pet.
- **Route & Arguments**: `Route.AdoptionApplication(val listingId: String)`
- **Access & Permissions**: Verified Adopters.
...
- **Target Endpoint(s)**: `POST /api/v1/applications`
...
```kotlin
data class AdoptionApplicationScreenUiState(
    val isLoading: Boolean = false,
    val error: String? = null
)

sealed interface AdoptionApplicationScreenUiEvent {
    object LoadData : AdoptionApplicationScreenUiEvent
}

@HiltViewModel
class AdoptionApplicationScreenViewModel @Inject constructor(
    private val repository: DefaultRepository
) : ViewModel() {
    private val _state = MutableStateFlow(AdoptionApplicationScreenUiState())
...
fun AdoptionApplicationScreen(
    viewModel: AdoptionApplicationScreenViewModel = hiltViewModel(),
...
> UI Design of an Adoption Application form. Dark mode. Input fields for describing experience with pets and why they want this specific dog. Cyan 'Submit Application' button.
