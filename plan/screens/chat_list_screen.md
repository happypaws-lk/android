...
- **Screen Name & ID**: `ChatListScreen`
- **Flow Position**: Main tab for inbox.
- **Route & Arguments**: `Route.ChatList`
...
- **Target Endpoint(s)**: `GET /api/v1/conversations`
...
sealed interface ChatListScreenUiEvent {
    object LoadData : ChatListScreenUiEvent
}
...
class ChatListScreenViewModel @Inject constructor(
    ...
    private val _state = MutableStateFlow(ChatListScreenUiState())
...
> UI Design of an Inbox/Chat List screen. Dark mode. A vertical list of chat threads. Each row shows an avatar, name, and preview of the last message. Unread threads have a small cyan dot.
