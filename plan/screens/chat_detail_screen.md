...
- **Screen Name & ID**: `ChatConversationScreen`
- **Flow Position**: Private messaging.
- **Route & Arguments**: `Route.ChatConversation(val conversationId: String)`
...
- **Target Endpoint(s)**: `GET /api/v1/conversations/{id}/messages`
...
```kotlin
data class ChatConversationScreenUiState(
    val isLoading: Boolean = false,
    val error: String? = null
)

sealed interface ChatConversationScreenUiEvent {
    object LoadData : ChatConversationScreenUiEvent
}

@HiltViewModel
class ChatConversationScreenViewModel @Inject constructor(
    private val repository: DefaultRepository
) : ViewModel() {
    private val _state = MutableStateFlow(ChatConversationScreenUiState())
...
fun ChatConversationScreen(
    viewModel: ChatConversationScreenViewModel = hiltViewModel(),
...
> UI Design of a Chat Conversation screen. Dark mode. Chat bubbles; sender messages are cyan (#4CE5E5) with black text, received messages are slate grey (#2B2B36) with white text. Message input field at bottom.
